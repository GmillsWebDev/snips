<script lang="ts">
  ...
  const service  = booking.services as unknown as { name: string; duration_minutes: number; price_pence: number }
  const customer = booking.customers as unknown as { first_name: string; last_name: string; email: string; phone: string }
