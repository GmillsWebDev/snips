...
export const load: PageServerLoad = async ({ locals, parent }) => {
  ...
  const upcomingBookings: UpcomingBooking[] = (upcomingResult.data ?? []).map(b => ({
    ...
    notes: b.notes ?? null,
    service: {
      name: b.services[0]?.name ?? '',
      durationMinutes: b.services[0]?.duration_minutes ?? 0,
      pricePence: b.services[0]?.price_pence ?? 0,
    },
    barberName: b.barbers[0]?.name ?? '',
    chairLabel: b.chairs[0]?.label ?? null,
    shopSlug: b.shops[0]?.slug ?? null,
  }))
  ...
  const pastBookings: PastBooking[] = (pastResult.data ?? []).map(b => ({
    ...
    service: { name: b.services[0]?.name ?? '' },
