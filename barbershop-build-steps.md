...
## 1 — Supabase Setup
- 1.1 Create project & configure environment variables
- 1.2 Write and run all schema migrations (tables, indexes, constraints)
- 1.3 Configure Row Level Security policies for all tables
- 1.4 Set up Supabase Auth (email/password + magic link)
...
- 2.1 Initialise project, folder structure, and route groups
- 2.2 Install and configure Supabase JS client
- 2.3 Set up auth session handling (hooks, layout load functions)
- 2.4 Build shared UI component library (buttons, modals, form inputs, toasts)
- 2.5 Configure route guards (public vs customer vs admin)

---

## 3 — Public Booking Flow
- 3.1 Shop landing page (`/book/[slug]`)
...
## 4 — Admin Core
- 4.1 Admin dashboard (today's bookings, quick stats)
- 4.2 Bookings list page (filterable by status, date, barber)
- 4.3 Single booking detail page
